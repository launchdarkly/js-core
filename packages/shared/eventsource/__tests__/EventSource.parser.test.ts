...
it('causes entire event to be ignored for empty event field', async () => {
  await withServer(async (server) => {
    ...
    // Spies on every dispatched type through a registry from createEventRegistry, since the
    // class no longer exposes a public dispatch method to monkey-patch.
    ...
    await withEventSource(server.url, { createEventRegistry: () => registry }, async (es) => {
      await shouldReceiveMessages(es, [{ data: 'Hello' }]);
