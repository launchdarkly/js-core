...
it('reads x-ld-fd-fallback-ttl on the error path (regression: SDK-2617)', async () => {
  const mockEventSource = createMockEventSource();
  const mockRequests = createMockRequests(mockEventSource);
  const base = createBase(mockRequests, logger);
  base.start();
...
it('defers the open fallback directive to the next changeSet (regression: SDK-2617)', async () => {
  const mockEventSource = createMockEventSource();
  const mockRequests = createMockRequests(mockEventSource);
  const base = createBase(mockRequests, logger);
  base.start();
...
it('emits terminal_error for a goodbye with protocolFallbackTTL (regression: SDK-2617)', async () => {
  const mockEventSource = createMockEventSource();
  const mockRequests = createMockRequests(mockEventSource);
  const base = createBase(mockRequests, logger);
  base.start();
...
it('emits terminal_error for a goodbye when a deferred open directive is pending (regression: SDK-2617)', async () => {
...
it('clears a pending fallback directive when onopen fires without the fallback header (regression: stale-pendingFallback)', async () => {
  const mockEventSource = createMockEventSource();
  const mockRequests = createMockRequests(mockEventSource);
  const base = createBase(mockRequests, logger);
  base.start();
...
it('close resets the pending fallback directive (regression: SDK-2617)', async () => {
