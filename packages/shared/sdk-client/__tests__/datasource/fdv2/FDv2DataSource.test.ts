...
it('re-engages FDv2 after the fdv1 fallback TTL expires', async () => {
  ...
  const fdv2Factory = jest.fn(() => {
    ...
    return fdv2Created === 1
      ? makeMockSynchronizer([changeSet(fdv2Payload, { fdv1Fallback: true, fdv1FallbackTtlMs: 50 })])
      : makeMockSynchronizer([changeSet(fdv2RecoveryPayload, { fdv1Fallback: false })]);
  });
  const fdv1Sync = makeMockSynchronizer([changeSet(fdv1Payload, { fdv1Fallback: false })]);
...
it('uses a default recovery interval (not zero) when fdv1FallbackTtlMs is absent (regression: SDK-2617 §8.3.2)', async () => {
  ...
  const fdv2Factory = jest.fn(() => {
    ...
    return fdv2Created === 1
      ? makeMockSynchronizer([changeSet(fdv2Payload, { fdv1Fallback: true })])
      : makeMockSynchronizer([changeSet(fdv2RecoveryPayload, { fdv1Fallback: false })]);
  });
