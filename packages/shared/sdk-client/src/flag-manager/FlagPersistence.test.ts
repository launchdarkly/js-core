import { Context, Hasher, LDContext, LDLogger, Platform, Storage, Crypto, PlatformData, SdkData, EventSource, EventSourceInitDict, Options, Response } from '@launchdarkly/js-sdk-common';
...
describe('FlagPersistence tests', () => {
  ...
  test('init successfully persists flags', async () => {
    ...
    const contextDataKey = namespaceForContextData(
      mockPlatform.crypto,
      TEST_NAMESPACE,
      context,
    );
  ...
  test('init prunes cached contexts above max', async () => {
    ...
    const context1DataKey = namespaceForContextData(
      mockPlatform.crypto,
      TEST_NAMESPACE,
      context1,
    );
    const context2DataKey = namespaceForContextData(
      mockPlatform.crypto,
      TEST_NAMESPACE,
      context2,
    );
  ...
  test('upsert ignores inactive context', async () => {
    ...
    await fpUnderTest.upsert(inactiveContext, 'inactiveContextFlag', {version: 1, flag: makeMockFlag()});
