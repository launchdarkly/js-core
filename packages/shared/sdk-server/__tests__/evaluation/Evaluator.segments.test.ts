...
describe.each([true, false])(
  'when evaluating user equivalent contexts for segments, cache: %p',
  (cache) => {
...
describe.each([true, false])(
  'Evaluator - segment match for non-user contexts, withCache(cache): %p',
  (cache) => {
    ...
    it('does not exceed callstack side for circular segments', async () => {
      ...
      const res = await evaluator.evaluate(
        flag,
        Context.fromLDContext(singleKind),
        undefined,
        withCache(cache),
      );
      expect(res.detail.reason).toEqual({ kind: 'ERROR', errorKind: 'MALFORMED_FLAG' });
