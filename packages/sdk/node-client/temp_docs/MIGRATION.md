...
is now configured via a `storage` option on `LDOptions`. The option accepts
any object that satisfies the `Storage` interface (`get`, `set`, `clear`),
replacing the previous `localStoragePath` field.

**Omitting `storage`** uses the built-in file-backed cache at
`<cwd>/ldclient-user-cache` -- no change needed for this case.
...
```typescript
import { createClient, createDefaultStorage } from '@launchdarkly/node-client-sdk';

const client = createClient(envKey, initialContext, {
  storage: createDefaultStorage({ localStoragePath: '/var/cache/ldclient' }),
});
```

`createDefaultStorage` returns the process-wide file-backed singleton for
that path, so multiple clients that share a cache directory stay in sync:

```typescript
const storage = createDefaultStorage({ localStoragePath: '/var/cache/ldclient' });
const client1 = createClient(envKey, context1, { storage });
const client2 = createClient(envKey, context2, { storage });
```
...
Custom implementations must not throw exceptions.
...
The previous SDK accepted the secure mode hash as a positional second
argument to `identify()`:

```javascript
// Before
client.identify(newContext, newHash, callback);
```

The new SDK accepts the hash both at construction time (as a static default)
and per-identify via `LDIdentifyOptions`. When a per-identify hash is
provided it takes precedence over the config-level value for that call and
all subsequent network requests until the next `identify`.

```typescript
// Static hash -- applies to all identifies that do not pass their own
const client = createClient(envKey, initialContext, { hash: 'default-hash' });
await client.start();

// Per-identify hash -- overrides the config hash for this call
await client.identify(newContext, { hash: 'per-context-hash' });

// No hash on this call -- falls back to 'default-hash'
await client.identify(anotherContext);
```

If your application never needs to rotate the hash, pass it once in
`LDOptions` and omit it from individual `identify` calls.

## Mobile key support

By default, `createClient` treats the first argument as a **client-side ID**.
To authenticate with a **mobile key** instead, set `useMobileKey: true`. When
enabled, the SDK uses the mobile-specific event paths (`/mobile`,
`/mobile/events/diagnostic`) and the mobile FDv1 data source endpoints
(`/msdk/evalx/...`, `/meval/...`), and sends the credential as an
`Authorization` header rather than embedding it in URLs.

```typescript
const client = createClient(mobileKey, initialContext, {
  useMobileKey: true,
});
await client.start();
```

`useMobileKey` defaults to `false`, so existing code that passes a
client-side ID continues to work without changes.

**Mobile keys do not support secure mode.** Setting both `useMobileKey: true`
and `hash` (the secure mode hash) causes `createClient` to throw at
construction time:

```typescript
// Throws: Invalid configuration: secure mode "hash" is not supported when "useMobileKey" is true.
createClient(mobileKey, initialContext, {
  useMobileKey: true,
  hash: 'some-hash',
});
```

If your application needs secure mode, keep `useMobileKey` at its default
(`false`) and pass a client-side ID.
