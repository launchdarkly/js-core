...
| ✅     | Initialization                  | `initialize` waits for the LaunchDarkly client with the `initTimeoutSeconds` constructor parameter, which defaults to 10 seconds. Zero does not wait, so readiness is reported through provider events, and null waits indefinitely.                                                                              |
