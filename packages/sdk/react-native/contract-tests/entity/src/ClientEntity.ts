...
function makeSdkConfig(options: SDKConfigParams, tag: string) {
  ...
  const cf: LDOptions = {
    ...
    // Disable automatic lifecycle handling for contract tests
...
export class ClientEntity {
  constructor(
    private readonly _client: ReactNativeLDClient,
    private readonly _logger: ReturnType<typeof makeLogger>,
  ) {}
...
export async function newSdkClientEntity(options: CreateInstanceParams) {
  ...
  try {
    ...
    // we get here if identify() rejects or if we timed out
    failed = true;
  }
