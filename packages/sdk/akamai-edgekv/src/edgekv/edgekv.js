...
export class EdgeKV {
  ...
  getNamespaceToken(namespace) {
    ...
    if (!(name in edgekv_access_tokens)) {
      throw (
        "MISSING ACCESS TOKEN. No EdgeKV Access Token defined for namespace '" + namespace + "'."
      );
    }
  ...
  addTimeout(options, timeout) {
    if (
      timeout &&
      (typeof timeout !== 'number' || !isFinite(timeout) || timeout <= 0 || timeout > 1000)
    ) {
  ...
  putRequest({
    ...
    let uri =
      this.#edgekv_uri + '/api/v1/namespaces/' + namespace + '/groups/' + group + '/items/' + item;
    return httpRequest(
