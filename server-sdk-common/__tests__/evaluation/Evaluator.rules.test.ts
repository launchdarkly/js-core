...
import { makeClauseThatDoesNotMatchUser, makeClauseThatMatchesUser, makeFlagWithRules } from './flags';
...
describe('when evaluating user equivalent contexts', () => {
  ...
  it.each<LDContext>([basicUser, basicSingleKindUser, basicMultiKindUser])('matches user from rules', async (userToTest) => {
    const rule0: FlagRule = {
      id: 'id0', clauses: [noMatchClause], variation: 1,
    };
...
describe('when evaluating non-user contexts', () => {
  ...
  it.each([singleKindContext, multiKindContext])('matches user from rules', async (contextToTest) => {
